...
func (b *NodeBuilderImpl) expandModuleDecl(symbol *ast.Symbol) *ast.Node {
	...
	for i := 0; i < len(members); i++ {
		...
		if resolved.Flags&(ast.SymbolFlagsFunction|ast.SymbolFlagsMethod) != 0 {
			...
			if !hasModuleExports {
				bodyStmts = append(bodyStmts, hoverStatement{node: b.f.NewModuleDeclaration(nil /*modifiers*/, ast.KindNamespaceKeyword, b.f.NewIdentifier(m.Name), nil /*attributes*/, b.f.NewModuleBlock(b.f.NewNodeList(nil)))})
			}
			continue
		}
	...
	return b.f.NewModuleDeclaration(nil /*modifiers*/, keyword, localName, nil /*attributes*/, b.f.NewModuleBlock(b.f.NewNodeList(bodyStatements)))
}
