...
var fixAllDescriptionMessages = map[string]*diagnostics.Message{
	fixIdUnusedIdentifierDelete:        diagnostics.Delete_all_unused_declarations,
	fixIdUnusedIdentifierDeleteImports: diagnostics.Delete_all_unused_imports,
	fixIdUnusedIdentifierInfer:         diagnostics.Replace_all_unused_infer_with_unknown,
	fixIdUnusedIdentifierPrefix:        diagnostics.Prefix_all_unused_declarations_with_where_possible,
}
...
func getAllUnusedIdentifierCodeActions(ctx context.Context, fixContext *CodeFixContext) (*CombinedCodeActions, error) {
	...
	changeTracker := fixer.createChangeTracker()
	seenFunctionDeletes := make(map[*ast.Symbol]bool)
		...
		if containsErrorCode(unusedIdentifierErrorCodes, diag.Code()) {
			...
			for _, info := range fixer.getInfo(diag.Code(), token) {
				if info.fixID == fixContext.FixID {
					if info.kind == unusedIdentifierFixKindDeleteFunctionDeclaration {
						symbol := info.node.AsFunctionDeclaration().Symbol
						if seenFunctionDeletes[symbol] {
							continue
						}
						seenFunctionDeletes[symbol] = true
					}
					fixer.writeChanges(changeTracker, info)
				}
			}
	...
	diagnostic := diagnostics.Delete_all_unused_declarations
	if msg, ok := fixAllDescriptionMessages[fixContext.FixID]; ok {
		diagnostic = msg
	}

	return &CombinedCodeActions{
		Description: diagnostic.Localize(locale.FromContext(ctx)),
		Changes:     changes,
	}, nil
}
...
func (f *unusedIdentifierFixer) createCodeAction(description string, fixID string, changes []*lsproto.TextEdit) *CodeAction {
	...
	if msg, ok := fixAllDescriptionMessages[fixID]; ok {
		fixAllDescription = msg.Localize(f.locale)
	}
	return &CodeAction{
		Description:       description,
		Changes:           changes,
